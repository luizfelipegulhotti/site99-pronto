...
<link rel="stylesheet" href="css/santos.css">
    ...
    <div class="main-layout">
        ...
        <!-- CARDS PARA MOBILE -->
        <div class="saints-cards" id="saintsCards"></div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const saintsCards = document.getElementById('saintsCards');
        ...
        // Função para criar cards para mobile
        function createSaintsCards() {
            saintsCards.innerHTML = '';
            items.forEach((item, index) => {
                ...
                card.innerHTML = `
                    <img src="${item.smallImage}" alt="${item.nome}" class="saint-card-image">
                    <div class="saint-card-content">
                        <div class="saint-card-title">${item.nome}</div>
                        <div class="saint-card-text">${item.text}</div>
                    </div>
                `;
        ...
        function createOrbitalItems() {
            ...
            const radius = orbitalWidth / 2 - 50;
            ...
            items.forEach((item, index) => {
                ...
                smallCircle.style.transform = `translate(-50%, -50%) rotate(${-rotationDegree}deg)`;
        ...
        function setActiveItem(index, animate = true) {
            ...
            if (isMobile) {
                // Atualizar cards ativos no mobile
                document.querySelectorAll('.saint-card').forEach((card, i) => {
                    card.classList.toggle('active', i === index);
                });
            } else {
                // Atualizar círculos ativos no desktop
                ...
                setTimeout(() => {
                    mainImage.src = items[index].largeImage || items[index].smallImage;
            ...
            // Atualizar imagem de fundo e texto (ambos mobile e desktop)
        ...
        // Event listeners para os botões de navegação (apenas desktop)
        if (nextBtn) nextBtn.addEventListener('click', goToNext);
        if (prevBtn) prevBtn.addEventListener('click', goToPrev);

        // Teclas de seta para navegação (apenas desktop)
